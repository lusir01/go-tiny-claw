// Package provider internal/provider/openai.go
package provider
...
func NewZhipuOpenAIProvider(model string) *OpenAIProvider {
	...
	if apiKey == "" {
		panic("ZHIPU_API_KEY environment variable is required. Please export it in your shell profile (~/.zshrc or ~/.bashrc)")
	}

	// 核心：将官方 SDK 的地址替换为智谱的兼容端点
	baseURL := os.Getenv("ZHIPU_BASE_URL")
	if baseURL == "" {
		baseURL = "https://open.bigmodel.cn/api/paas/v4/v1" // 智谱官方默认地址
	}
